...
public class Utils {
    ...
    public static void openShulkerBox(Player player, ItemStack stack, InteractionHand hand) {
        if (!player.level().isClientSide() && player instanceof ServerPlayer) {
            ShulkerBoxHand shulkerBox = createShulkerBox(player, stack, hand);
            shulkerBox.applyComponentsFromItemStack(stack);
            shulkerBox.openShulker(player);
        }
    }

    private static ShulkerBoxHand createShulkerBox(Player player, ItemStack stack, InteractionHand hand) {
        BlockPos pPos = player.getOnPos();
        BlockState pBlockState = ((BlockItem) stack.getItem()).getBlock().defaultBlockState();
        return new ShulkerBoxHand(pPos, pBlockState, player, stack, hand);
    }
